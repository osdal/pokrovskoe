...
jQuery(document).ready(function(){
	...
	//menu		nav navbar-nav
	jQuery('.nav.navbar-nav li.dropdown > ul.dropdown-menu').animate({'opacity': 0, 'top': '200%'});

  jQuery('.nav.navbar-nav > li.dropdown').hover(function () {
    var currentItem = jQuery(this).find('ul:first');
    currentItem.stop().css({display:'block'}).animate({'opacity': 1, 'top': '100%'}, 300);
  },function() {
    var currentItem = jQuery(this).find('ul:first');
    currentItem.stop().animate({'opacity': 0, 'top': '200%'}, 300, function()
      {
  ...
  if (jQuery('html').attr('dir') == 'rtl') {
    jQuery('.nav.navbar-nav li.dropdown > ul.dropdown-menu ul.dropdown-menu').animate({'opacity': 0, 'top': '-4px', 'right': '100%'});
    jQuery('.nav.navbar-nav > li.dropdown li.dropdown').hover(function () {
      var currentItem = jQuery(this).find('ul:first');
    ...
    jQuery('.nav.navbar-nav li.dropdown > ul.dropdown-menu ul.dropdown-menu').animate({'opacity': 0, 'top': '-4px', 'left': '100%'});
    jQuery('.nav.navbar-nav > li.dropdown li.dropdown').hover(function () {
      var currentItem = jQuery(this).find('ul:first');
	...
	jQuery('ul.nav.navbar-nav:eq(0)').mobileMenu({
	    switchWidth: 960,                                 //width (in px to switch at)
	    topOptionText: jQuery('ul.nav.navbar-nav a.active').text(),       //first option text
